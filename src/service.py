...
class Model(object):
    ...
    def search(self, smiles_list): # <-- EDIT: rename if model does not do predictions (e.g. it does calculations)
        ...
        with open(run_file, "w") as f:
            lines = [
                "bash {0}/run_search.sh {0} {1} {2}".format( # <-- EDIT: match method name (run_predict.sh, run_calculate.sh, etc.)
                    self.framework_dir,
                    data_file,
                    output_file
                )
        ...
        with open(output_file, "r") as f:
            ...
            R = []
            for r in reader:
                R += [{"outcome": [String(x) for x in r]}] # <-- EDIT: Modify according to type of output (Float, String...)
        meta = {
            "outcome": h
        }
        result = {
...
class Service(BentoService):
    ...
    def search(self, input: List[JsonSerializable]): # <-- EDIT: rename if necessary 
        input = input[0]
        smiles_list = [inp["input"] for inp in input]
        output = self.artifacts.model.search(smiles_list) # <-- EDIT: rename if necessary
        return [output]
